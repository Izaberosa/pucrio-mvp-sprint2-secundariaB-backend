...
info = Info(title="Vivara", version="1.0.1")
...
def add_usuario(form: UsuarioSchema):
    ...
    usuario = Usuario(
        ...
        email = form.email,
        joia = form.joia)
    logger.debug(f"Adicionando usuario nome: '{usuario.nome}'")
    try:
        ...
        session.commit()
        logger.debug(f"Adicionado usuario  nome: '{usuario.nome}'")
        return apresenta_usuario(usuario), 200

    except IntegrityError as e:
        # como a duplicidade do nome é a provável razão do IntegrityError
        error_msg = "Esse usuário já tem no banco :("
